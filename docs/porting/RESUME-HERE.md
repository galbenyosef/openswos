...
**Last updated: 2026-07-18 (Session 34: team-picker tiles + drill-down, clock=54, stamina 1.67 + tackle/keeper fatigue, faithful ball control (flag), i18n 20 languages, v0.3 release w/ multi-platform builds. BLOCKED: R36S build still crashes — NEXT STEP below. Still next: footer-hint i18n keys, #237, #238, #216.)**

## >>> RESUME FIRST — R36S CRASH DEBUG (session 34, BLOCKED, do this before anything else)
OpenSWOS on the user's R36S (ArkOS) shows the R36S logo then **exits immediately** (crash). Fixed the obvious cause (ArkOS userland is **32-bit armhf**, not aarch64 — confirmed by web search + StardewValley PortMaster mounting aarch64 mono; our ARM64 binary lacked 64-bit system libs). Built a **Linux ARM32 (armhf)** export (preset `[preset.5]` "Linux ARM32" in `game/export_presets.cfg`, `architecture="arm32"`; verified `ELF 32-bit ARM, interpreter /lib/ld-linux-armhf.so.3`). Repackaged as `OpenSWOS-v0.3-R36S-ArkOS.zip` (launcher modeled on the user's working `G:\ports\wiliwili.sh` — sets HOME, LD_LIBRARY_PATH, SDL_GAMECONTROLLERCONFIG_FILE, logs to `openswos.log`), uploaded to the v0.3 release, deleted the broken ARM64-R36S asset. **User reports the arm32 build ALSO crashes the same way.**

**DO NOT KEEP GUESSING. Get the real error:** the launcher writes stderr to **`G:\ports\OpenSWOS\openswos.log`** on the user's games SD card (mounted at `G:\`, ArkOS ports live in `G:\ports\`). When the user puts the card back in the PC, **READ `G:\ports\OpenSWOS\openswos.log` directly** — it holds the exact crash reason (missing lib, GLES/EGL context fail, .NET load error, etc.). Ask the user to run it once then reinsert the card if the log isn't there yet. `G:\ports\` also has working native ports (`wiliwili.sh`, `StardewValley.sh`, `ClassiCube.sh`, `PortMaster/`) to copy patterns from — the user said DO NOT modify anything in `G:\ports\`, only fix our zip. Likely remaining suspects once the log is read: GLES2/GLES3 driver (Godot gl_compatibility on Mali-G31 panfrost — may need `MESA_GL_VERSION_OVERRIDE`/`SDL_VIDEODRIVER`), missing 32-bit lib, or the .NET/Mono runtime not initialising on armhf. Build/repack helper paths: arm32 export dir `I:\GITHUB\W_OPEN_SWOS\build\linux-arm32\`, port staging `I:\GITHUB\W_OPEN_SWOS\build\r36s\`, packaged zip `I:\GITHUB\W_OPEN_SWOS\build\release\OpenSWOS-v0.3-R36S-ArkOS.zip`.
