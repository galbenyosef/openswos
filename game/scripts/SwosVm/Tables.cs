...
// Lookup tables embedded in swos-port and the original SWOS binary. These are
// referenced by ported functions like CalculateDeltaXAndY (motion vector solve)
// and updateBall (frame index selection). Copied verbatim — every value
// reproduces what the binary uses.
//
// Sources:
//   - kSineCosineTable: external/swos-port/src/sprites/updateSprite.cpp:56 (also swos.asm:236334)
//   - kAngleTangent:    external/swos-port/src/sprites/updateSprite.cpp:16
public static class Tables
{
    // Sine/cosine table — input is 0..255 representing an angle (256 = 1 full circle).
    // Output is approx 32_767 * sin(x / 256 * 2π), SWOS fixed-point format.
    // Wrap-around: cosine = kSineCosineTable[(angle + 64) & 0xFF].
    public static readonly short[] kSineCosineTable = new short[256]
    {
    ...
    // Angle-tangent table — kAngleTangent[deltaY][deltaX] returns an angle 0..64
    // representing the direction (deltaX, deltaY) for x >= y. For x < y, the
    ...
    public static readonly sbyte[,] kAngleTangent = new sbyte[32, 32]
    {
